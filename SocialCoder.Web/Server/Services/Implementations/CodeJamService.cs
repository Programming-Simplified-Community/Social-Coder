...
public class CodeJamService : ICodeJamService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        this._context.CodeJamTopics.Add(topic);
    ...
    public async Task<ResultOf> Delete(int topicId, CancellationToken cancellationToken = default)
    {
        ...
        this._context.CodeJamTopics.Remove(topic);
        await this._context.SaveChangesAsync(cancellationToken);
    ...
    public async Task<ResultOf<CodeJamTopic>> AdminUpdateTopic(CodeJamTopic topic, CancellationToken cancellationToken = default)
    {
        ...
        var item = await this._context.CodeJamTopics.FirstOrDefaultAsync(x => x.Id == topic.Id, cancellationToken);
        ...
        this._context.CodeJamTopics.Update(item);
        await this._context.SaveChangesAsync(cancellationToken);
    ...
    public async Task<ResultOf<CodeJamViewModel>> Register(CodeJamRegistrationRequest request, string? userId, CancellationToken cancellationToken = default)
    {
        ...
        var check = await (from topic in this._context.CodeJamTopics
                           where topic.Id == request.TopicId && topic.IsActive

                           let reg = (from reg in this._context.CodeJamRegistrations
        ...
        this._context.CodeJamRegistrations.Add(new()
        {
            UserId = userId,
    ...
    public async Task<ResultOf<CodeJamViewModel>> Abandon(CodeJamAbandonRequest request, string? userId, CancellationToken cancellationToken = default)
    {
        ...
        var resultSet = await (from reg in this._context.CodeJamRegistrations
                               join topic in this._context.CodeJamTopics
        ...
        {
            this._context.CodeJamRegistrations.Remove(resultSet.Registration);
            await this._context.SaveChangesAsync(cancellationToken);
    ...
    public async Task<ResultOf<CodeJamViewModel>> GetTopic(int topicId, string? userId,
        CancellationToken cancellationToken = default)
    {
        var item = await (from topic in this._context.CodeJamTopics
                          where topic.Id == topicId

                          let isRegistered = (from reg in this._context.CodeJamRegistrations
                                              where reg.CodeJamTopicId == topicId && reg.UserId == userId && reg.AbandonedOn == null
                                              select reg).Any()
                          let soloApps = (from reg in this._context.CodeJamRegistrations
                                          where reg.CodeJamTopicId == topicId && !reg.PreferTeam && reg.AbandonedOn == null
                                          select reg).Count()

                          let total = (from reg in this._context.CodeJamRegistrations
