...
public class UserService : IUserService
{
    ...
    public async Task<ResultOf> BanUser(BanUserRequest request, string callingUserId, CancellationToken cancellationToken = default)
    {
        ...
        // Find this user
        var existingBan = await this._context.PlatformBans
            .FirstOrDefaultAsync(x => x.UserId == request.UserId, cancellationToken);

        if (existingBan is null)
        {
            this._context.PlatformBans.Add(new PlatformUserBan
            {
        ...
        else
        {
            ...
            this._context.PlatformBans.Update(existingBan);
        }

        try
        {
            await this._context.SaveChangesAsync(cancellationToken);
            this._logger.LogInformation("{Username} was banned by {Banner} for '{Reason}'",
                user.UserName,
                banner.UserName,
                request.Reason);
        ...
        {
            this._logger.LogError("Was unable to ban {Username} by {Banner} for '{Reason}' due to an exception: {Error}",
                user.UserName,
                banner.UserName,
                request.Reason,
                ex);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (result.Succeeded)
        {
            this._logger.LogInformation("{User} was granted {Role} by {Admin}",
                user.UserName,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (result.Succeeded)
        {
            this._logger.LogInformation("Removed {RoleName} from {User} by {Admin}: '{Reason}'",
                request.RoleName,
                user.UserName,
                callingUser.UserName,
                request.Reason);
    ...
    public async Task<ResultOf<ApplicationUser>> GetUserFromOAuth(AuthenticateResult authResult)
    {
        ...
        {
            this._logger.LogError("Was unable to create new user from {Scheme}\n{Error}",
                authProvider,
                string.Join("\n", createResult.Errors.Select(x => x.Description)));
            return ResultOf<ApplicationUser>.Fail("Was unable to create user");
        }

        var addLoginResult = await this._userManager.AddLoginAsync(user, new UserLoginInfo(
            authProvider,
            userIdClaim.Value,
            authProvider));
        ...
        if (this.HasClaim(ref claims, ClaimConstants.GithubName, out var githubUsername))
        {
        ...
        */
        ...
        var roleResult = await this._userManager.AddToRoleAsync(user, Roles.Owner);

        if (!roleResult.Succeeded)
        {
            this._logger.LogError("Was unable to add {Role} to {User}. {Error}",
                Roles.Administrator,
                name,
        ...
        {
            this._logger.LogInformation("Added {User} to {Role}", name, Roles.Administrator);
        }
